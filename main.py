...
def changeMotorSpeed(velocityArray):  # FR, FL, BL, BR
	...
	r.servo_board.direct_command('m', 1,0)
...
def move(direction, speed, time, degrees=True, autoStop=True):  # Magic
	...
	motor0 = (speed * mfrPower / normalisationFactor) + 0 * sign(mfrPower)
	motor1 = (speed * mflPower / normalisationFactor) + 0 * sign(mflPower)
	motor2 = (speed * mblPower / normalisationFactor) + 0 * sign(mblPower)
	motor3 = (speed * mbrPower / normalisationFactor) + 0 * sign(mbrPower)
	
	velocityArray = [mapThing(motor0,0,20,10,20),mapThing(motor1,0,20,10,20),mapThing(motor2,0,20,10,20),mapThing(motor3,0,20,10,20)]
...
def moveByLocalVec(moveVec, maxSpeed=50, minSpeed=10):  # Magic
...
def updatePosition(force=True):  # Magic, but modified to make it easier to read and put all in one loop
	cameraAngleArray = [0, 0, 45, -45, -90, 90]
	mNumber = 0
	loopNumber = 0
	while mNumber <= 0:
...
def initMarkers():
	...
	for position in [Vector(4, 6), Vector(6, 4), Vector(4, 2), Vector(2, 4)]:
		norm = 0
		for vectorToAdd in [Vector(0, 0.185), Vector(0.185, 0), Vector(0, -0.185), Vector(-0.185, 0)]:
...
def main():  # Just for testing really, in game just run playGame()
	...
	for i in range(0, 360, 10):
		move(i, 20, 0.5, True, autoStop= False)
		r.power_board.buzz(0.4, frequency = 200)
...
if r.mode == GameMode.DEVELOPMENT:
	main()


# To be run when start button pressed
def playGame():
	...
	moveTo(firstMoves[robotZone][0], (startLookDirections[robotZone] + 45) % 360)
	moveTo(firstMoves[robotZone][1], (startLookDirections[robotZone]+180) % 360)
	...
	while tokensCollected <= 5:
		closestToken = None
		tokenLoopCount = 0
		while closestToken is None and tokenLoopCount < 8:
			closestToken = getClosestMarker(tokenType)
			turnByAngle(20, 45)

		if closestToken != None:
			if closestToken.grab():
				tokensCollected += 1
			else:
	...
	while tokensCollected != 5:
		...
		tokenLoopCount = 0
		while closestToken is None and tokenLoopCount < 8:
			closestToken = getClosestMarker(oppositeTokenType)
			turnByAngle(20, 45)
	...
	turnToFaceVec(20, Vector(6, 6))
	moveTo(Vector(6, 6))
	raiseArm()

	turnToFaceVec(20, Vector(2, 2))
	moveTo(Vector(2, 2))
